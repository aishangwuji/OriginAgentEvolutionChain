# EC-14 Contribution Points Sandbox Validation Result

Date: 2026-05-23

## Scope

EC-14 adds a non-transferable `ContributionPointsLedger` and SDK artifacts for Contribution Points grant, consume, and deny decisions.

It does not add ERC20 behavior, marketplace settlement, real Points value, staking, Credit Deduction, reward vaults, Commitment vaults, or DAO governance.
...
```text
Contribution Points action/report hash stability
...
grant/consume event linkage against ContributionPointsLedger events
missing optional ContributionPointsLedger deployment error
...
`ContributionPointsLedger` does not enforce trust policy on-chain. EC-13 gates are enforced by SDK/runner workflow before broadcast. The contract remains a minimal owner-only accounting ledger.
