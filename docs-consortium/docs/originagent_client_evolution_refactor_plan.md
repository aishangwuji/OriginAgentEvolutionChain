...
EC-18+：再评估真实 Points、staking、Credit Deduction、DAO、ContributionPool、CreditPool。
```
...
sandbox.py:
  确定性沙箱。
  限制网络、文件、shell、Points、工具调用次数、运行时长。
...
```text
Points pre-flight 超额：调用前拒绝
Points post-flight 超额：冻结后续调用并记账
工具循环立即停止
越权访问立即停止
异常网络请求立即停止
状态写入越界立即停止
...
permissions、external_endpoints、Points_budget 可静态检查。
...
#### Phase 1-G：脱敏遥测、Points 预算与本地 proof bundle（已完成）
...
实现 Points budget pre-flight / post-flight 预算边界。
...
```text
错误、拒绝、回滚和 Points 超额只记录脱敏摘要。
Points 预算超额能阻断执行路径并写入结构化事件。
...
可以生成脱敏遥测摘要。
可以生成本地 proof bundle。
可以执行 Points 预算 pre-flight / post-flight。
...
### 18.2 Points 预算控制的物理延迟

风险：

```text
LLM Provider 的精确 Points 消耗通常是后验统计。
Streaming 场景下，实际 Points 可能到 chunk 返回后才知道。
...
```text
Pre-flight Check:
  调用 Provider 前估算 prompt / tool payload / attachment 的 Points 数。
  超过预算或单次请求上限时直接拒绝。

Post-flight Accounting:
  Provider 返回后按真实 Points 记账。
...
```text
外部 Provider 调用前必须经过 Pointsizer 或保守估算器。
manifest 的 Points_budget 不只是报告字段，必须参与运行时熔断。
单次请求上限和累计预算上限要分开记录。
```
...
```text
不做 per-provider 精准 Pointsizer。
使用保守估算器：
  ceil(char_count / 3.5)

如果 Provider 提供 Points counting API：
  优先使用 Provider 官方计数。

Provider 返回后：
  从 API response usage 字段读取真实 Points 并记账。
```
...
```text
累计预算：
  manifest.context_budget.Points_budget

单次请求上限：
  manifest.context_budget.Points_budget * 0.8
...
Phase 1 不要求积分钱包、不要求 DID、不要求 KYC。
...
```text
Python 第三方库的 Exception Message 可能包含 URL、Points、路径、用户输入或请求参数。
...
测试样例必须覆盖 URL Points、Bearer Points、本地路径和用户输入片段。
```
...
测试覆盖越权读写、网络、exec、provider、Points budget。
```
...
但遥测脱敏规则应更偏向 secret / URL / path / Points / prompt 泄漏。
公开摘要只写结构化字段。
原始日志只留本地私有区域。
```
...
旧问题原则上已解决。
第二轮新增问题主要集中在事件 schema、路径、身份、Points 估算、状态机委托等更细的工程细节。
```
...
### 20.8 中等问题 M7：Points pre-flight Pointsizer 未指定
...
```text
现有 memory.py 中使用 tikPoints.get_encoding("cl100k_base")。
该方案更接近 OpenAI 模型，不适用于所有 Provider。
Anthropic、Google、DeepSeek、本地模型的 Pointsizer 都可能不同。
```

风险：

```text
使用错误 Pointsizer 估算 Points，可能导致预算偏差。
恶意模块可能利用偏差发起过长请求。
```
...
Post-flight:
  Provider 返回后，从 response usage 字段读取真实 Points 并记账。
```
...
```text
Points pre-flight 估算：
  Phase 1 不使用 per-provider Pointsizer。
  使用保守估算器 ceil(char_count / 3.5)。
  Provider 返回后使用真实 Points count 记账。
  累计预算来自 manifest.context_budget.Points_budget。
  单次请求上限默认为 manifest.context_budget.Points_budget * 0.8。
```
...
边界不变：
  ...
  客户端不能把 Points 余额当成 trust，也不能把 transferable Points 当成 reputation。
...
| sanitized telemetry | telemetry digest + selected public counters | Points、latency、failure class 等可公开字段进入报告 |
...
在 Anvil / PVE Private Devnet 跑通最小 EVM 合约闭环，不在早期启动最终 private appchain，不发行真实积分。
```
...
ContributionPointsLedger / SimulatedBond:
  只做 Contribution Points 或模拟 bond 记账。
  不做真实担保、信用扣除、提现或可转让资产。
```
...
### 22.4 Phase 4：Contribution Points 激励模拟、信誉与挑战机制

目标：

```text
在测试网或 L2 上验证 Contribution Points 奖励模拟、身份信誉权重、挑战结果和社区 claim 质量是否能形成正反馈。
不引入真实 Points、真实担保、真实 Credit Deduction、ContributionPool 或 CreditPool。
```
...
ContributionPointsLedger:
  记录非转让 Contribution Points grant / consume / deny。

GrowthRewardReport:
  记录 epoch reward pool、node weights、Contribution Points grants 和 capped / blocked reasons。
...
挑战成功能影响模块状态、Contribution Points eligibility、reputation 和后续展示权重。
长期稳定运行、低回滚率、真实任务收益比单次高分更重要。
不产生真实 Points、staking、Credit Deduction 或 marketplace settlement。
```
...
把公共测试网、Contribution Points 或 Node ID 当成本地安全保证。
```
...
对未来适配的影响：
  ...
  客户端不应自行把 Points 余额、owner 地址或单次 checkpoint 当成可信度最终裁决。
...
```text
EC-11 方向已从“Contribution Points + 模块市场 MVP”前移为“Evolution Unit Kind Registry / 开放式可进化单元协议”。
...
边界：
  EC-11 不要求客户端接入真实 Points、staking、DAO 或公网测试网。
...
边界：
  EC-12 不做真实攻击、不扫描公网、不引入 Points / staking / DAO。
...
边界：
  EC-13 不新增合约、不引入 Points / staking / Credit Deduction / DAO。
  EC-13 不修改客户端运行时安全模型。
  Trust gate 是审计建议，不是客户端强制策略。
```

## 27. EC-14 Contribution Points Sandbox 对客户端的影响

2026-05-23 更新：

```text
EC-14 链侧阶段新增非转让 Contribution Points 沙盒账本，不要求 OriginAgent 客户端改动。

对客户端的直接影响：
  客户端仍保持离线可用。
  客户端不需要依赖 ContributionPointsLedger 才能启动、恢复、activate、rollback 或导出 proof bundle。
  客户端不应把 Contribution Points 当成真实余额、真实 Points 或自动信任凭证。

对未来适配的影响：
  客户端未来展示链侧模块、Passport 或 marketplace dry-run 状态时，可以显示 Contribution Points eligibility / balance 摘要。
  Contribution Points 只表示沙盒准入和审计状态，不代表真实经济权益。
  对 blocked / manual_review 主体，客户端 UI 应解释为“链侧沙盒准入受限”，而不是自动删除本地模块或记忆。
  客户端如生成可用于 Contribution Points 的 adoption / audit_request artifact，仍必须避免 prompt、facts、raw telemetry、本地路径、URL query、secret-like string。

边界：
  Contribution Points 不可转让。
  不支持提现、交易、担保或 Credit Deduction。
  EC-14 不修改客户端运行时安全模型。
...
核心原因：
  进化链已经有 audit、abuse report、trust gate 和 Contribution Points sandbox。
  但还没有 genesis/endpoint manifest、endpoint discovery、first-class node_id、节点活动、节点有效贡献和节点增长奖励模拟。
  如果直接做“注册节点随时间拿 Points”，会诱导 Sybil、空跑节点、刷上传和 referral farming。
```
...
客户端不应把节点奖励、Contribution Points 或未来 Points 当作本地安全保证。
```
...
客户端如果展示节点，应展示贡献摘要、trust gate、abuse warning 和 Contribution Points sandbox 状态，而不是只展示注册时间或邀请人数。
客户端可以显示节点 attribution，但不能把节点奖励、Contribution Points 或 Node ID 当成本地安全保证。
...
EC-15 仍只应使用 Contribution Points 模拟，不引入真实 Points、staking、Credit Deduction 或 DAO。
...
```text
网络发现、artifact 下载、社区报告和验证验证工作都不能成为上传本地 prompt、facts、memory、API key、session 或本地日志的理由。
...
客户端不能把 Contribution Points、未来 bond、validator 地址或 Node ID 当成本地安全保证。
```
...
challenge bond 仍是 Contribution Points 链下 artifact，不是客户端真实余额或退款保证。
...
客户端影响不变：
  ...
  Contribution Points challenge bond 仍不是客户端余额、真实退款或真实信用扣除。
```
