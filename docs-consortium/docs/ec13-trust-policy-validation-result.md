...
It does not add contracts, write chain state, change address-level `testnetReputation`, change Passport reputation checkpoints, or create Points/Credit Deduction behavior.
...
EC-13 outputs advisory gates only. It is safe to use as an EC-14 input, but EC-13 itself does not enforce Contribution Points eligibility, module listing, validator weighting, or artifact quarantine on-chain.
