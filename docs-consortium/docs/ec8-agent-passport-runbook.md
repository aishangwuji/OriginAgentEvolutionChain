...
It does not store memory, encrypted memory, private keys, Points balances, rewards, or recovery material.
...
EC-8 Passport is not an encryption key, Points, NFT, ownership marketplace, or natural-person proof. OriginAgent can continue to run without chain access. Recovery, encrypted memory vaults, non-transferable reputation, Contribution Points, and market economics are later phases.
...
The quota should apply to formally admitted subjects, not arbitrary wallet addresses. Historical Passports remain auditable; retired Passports should not keep earning work weight or Contribution Points weight.
