# EC-8 Note: Agent Passport, Encrypted Memory, Reputation, And Points Economy
...
```text
An Agent should have a long-lived identity, portable encrypted memory, non-transferable reputation, and eventually Points-mediated economic activity.
```

This note records the architectural decision so the project does not later collapse identity, memory, reputation, and Points incentives into one unsafe mechanism. The EC-8 implementation now covers the Agent Passport identity anchor only; encrypted memory, reputation, and contribution points economics remain later phases.
...
Current EC-8 implementation does not limit the number of Passports that one owner can register. This is a known future risk if Agent Passport is later connected to work weight, Contribution Points weight, validator weight, or rewards.
...
Historical Passports must remain auditable. Retired Passports should not keep earning work weight, Contribution Points weight, or reward eligibility.

## Reputation And Points Separation

Reputation and Points must remain separate.
...
Buying Points should not buy reputation. Otherwise the protocol becomes capital-weighted instead of contribution-weighted.
...
## Points Economy Guardrails

Do not give every registered Agent a fixed daily Points reward merely for existing. That creates an immediate sybil faucet.
...
keep all of this sandbox-only until real abuse data exists
```

Possible Points uses:
...
EC-11:
  Contribution Points and module market MVP.
  Use Contribution Points for module purchases, validation fees, and challenge bonds.
  Keep all economics sandbox-only.
...
EC-13+:
  Only after real abuse data: consider real Points, staking, rewards, DAO, and richer marketplace governance.
```
...
Do not make transferable Points balance equal trust.
Do not let OriginAgent require chain access to run locally.
Do not introduce real-contribution points economics before PVE private devnet / private appchain abuse data and operations data exist.
```
...
The idea is strategically valuable, but it should be introduced as a staged identity and portability track rather than as an immediate Points faucet or memory-on-chain design.
...
Points is transferable but rewards contribution, not existence.
