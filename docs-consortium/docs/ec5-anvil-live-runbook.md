...
EC-5 turns the EC-4 dry-run and offline audit flow into a local Anvil transaction loop. It does not add public testnet deployment, Points rewards, Credit Deductionable Commitment, DAO voting, or Production Network reputation.
...
On the remote machine, install Foundry if `forge/anvil/cast` are missing, then run the same script from a clean uploaded project copy. Remote EC-5 validation proves the local-chain transaction path on an independent host; it still does not create economic rights or Production Network reputation.
