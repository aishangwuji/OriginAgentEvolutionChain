...
EC-4 turns EC-3 evidence into a sandbox-only challenge workflow. It does not add Points rewards, Credit Deductionable Commitment, DAO voting, or Production Network reputation.
...
The challenge summary invalidates upheld evidence and applies EC-4 contribution standing deltas. `evidence-summary --challenge-summary` then removes invalidated evidence and blocks active reports from reporters with reputation `<= -10`.
