...
The artifact proves only that a separate runner produced a public validation receipt. It is not a privacy leak and it is not Production Network reputation.

## EC-3 Boundary

Validator reports are high-weight only after foundation allowlist and source diversity checks. Reports from the same `operator_group_hash` or `runner_fingerprint_hash` are capped by the SDK summary. Invalidated reports are excluded from summary weight and do not trigger points credit deduction in EC-3.
