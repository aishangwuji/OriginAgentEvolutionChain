...
一个正式 Developer / Validator / Operator 默认最多 3 个 Active Agent：主力 Agent、验证 / 测试 Agent、沙箱 / 实验 Agent。历史 passport 不能删除，应保留审计轨迹；退休 passport 不应继续获得工作权重或 Contribution Points 权重。
...
定义网络发现、入口 gateway、artifact mirror、社区工作节点、活动报告和 Contribution Points 增长奖励模拟。
建立 reward weight / anti-sybil calibration，但仍不引入真实 Points、staking、Credit Deduction、DAO 或 marketplace settlement。
...
```text
EC-16 完整版只模拟 Contribution Points 增长权重，不做真实结算。
...
不引入真实 Points。
不引入 staking / Credit Deduction。
不引入持币分红或节点被动收益。
不把 Contribution Points 宣传为金钱或空投预期。
```
...
如果 invitee 提交恶意模块、伪造证据、恶意挑战或严重失职：
  invitee 直接扣 reputation / Contribution Points / future bond。
  sponsor 按 sponsorLiability 承担轻量连带责任。
  多次担保失败后冻结 sponsor inviteQuota 或降级资格。

早期只建议使用 Contribution Points / reputation / quota freeze。
真实 Credit Deduction / Points penalty 必须等 EC-21+ 法务、审计和经济模型完成后再评估。
```
...
真实 Points bond / Credit Deduction 必须后置到 EC-21+。
```

不做：

```text
不做 Points holder revenue share。
不做持币分红。
不做 buyback 承诺。
不做邀请返利。
不做注册验证工作。
...
utilityPoints = 功能型积分。
staking = 担保。
Credit Deduction = 信用扣除。
DAO = 去中心化治理。
ContributionPool = 奖励金库。
CreditPool = 担保金库。
...
```text
不能把 Contribution Points 宣传为金钱或空投预期。
不能承诺 Points holder revenue share。
