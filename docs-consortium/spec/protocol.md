...
Evidence reports use `report_hash = sha256(canonical_json(evidence_report_without_report_hash))`. Evidence is sandbox-only by default and must not be treated as transferable Production Network reputation.
...
## EC-4 Challenge and contribution standing

EC-4 adds challenge adjudication for active evidence. It is sandbox-only audit state and does not create Points rewards, Credit Deductionable Commitment, or Production Network reputation.
...
Any address may submit a challenge against active evidence. The foundation owner is the only resolver in EC-4. A successful challenge invalidates the evidence. contribution standing deltas are:
...
Challenge records use schema version `originagent.evolution.challenge_record.v1`. Challenge summaries use `originagent.evolution.challenge_summary.v1` and expose only public hashes, addresses, statuses, invalidated evidence IDs, rejected challenge IDs, and contribution standing deltas.

`evidence-summary --challenge-summary` excludes challenge-invalidated evidence and blocks reports from reporters whose contribution standing is `<= -10`. Source diversity and runner diversity rules remain unchanged.
