...
- challenge/invalidate is state-only and testnet-only; no token slash exists in EC-3
...
- testnet reputation is audit-only and does not migrate to real economic weight
...
- chain-state-check reads module, evidence, challenge, identity, and testnet reputation state back from the chain
...
- encrypted memory vaults, reputation, token economics, and owner recovery remain later phases
...
- EC-10 does not add token economics, staking, DAO governance, or natural-person uniqueness
...
- EC-11 does not add Test Credit, marketplace settlement, token voting, staking, or DAO governance

EC-12 adds an adversarial simulation harness:

- abuse reports flag protocol-layer risks before Test Credit or marketplace incentives
...
- EC-12 does not add contracts, tokens, staking, slashing, DAO governance, or public-network attacks
...
- EC-13 does not write chain state, change reputation, slash, reward, or enforce Test Credit

EC-14 adds a non-transferable Test Credit sandbox:

- TestCreditLedger records owner-only grant/consume accounting by Agent Passport
- Test Credit is not ERC20 and has no transfer, approve, allowance, withdraw, or market behavior
- SDK Test Credit actions consume EC-13 trust gates before broadcast; blocked/manual-review subjects produce deny artifacts
...
- challenge bond remains an off-chain Test Credit artifact only, with no on-chain credit movement
...
- the protocol can anchor module, evidence, challenge, Passport, memory-vault, reputation, unit-kind, abuse, trust-policy, and Test Credit public facts
...
- it cannot yet identify work nodes as first-class registry subjects, assign formal developer/tester admission, pay uptime/contribution rewards, run marketplace settlement, or issue real tokens
- the recommended next design direction is to split future work into Work Node Registry, reward-weight simulation, anti-Sybil calibration, role admission, module publication gates, company admin recovery, and revenue-routing dry runs before any real token economics
...
The current network release direction is conservative but explicit: the long-term target is an OriginAgent self-operated private appchain, with Cosmos EVM and EVM appchain frameworks as candidate directions. Early network tests stay inside a PVE private devnet, while public testnet, full public source release, real token, staking, slash, DAO, and marketplace settlement remain postponed until network identity, endpoint discovery, artifact distribution, node work reports, reward simulation, and private-appchain operations are proven. See `docs/appchain-and-network-operations-plan.md`.
...
EC-14 test credit sandbox artifacts are generated under `out/ec14-test-credit-sandbox-flow/` and stay out of git. See `docs/ec14-test-credit-runbook.md` and `docs/ec14-test-credit-validation-result.md`.
