# EC-14 Test Credit Sandbox Runbook

Date: 2026-05-23

## Purpose

EC-14 adds a non-transferable Test Credit sandbox ledger before marketplace settlement, real tokens, staking, slashing, or DAO governance.

Test Credit is bound to `passportId`. It is not ERC20 and has no transfer, approve, allowance, withdraw, or secondary-market behavior.

## Contract

`TestCreditLedger` stores:
...
EC-12 and EC-13 did not add contracts. The deployment `contractVersion` therefore moves from `ec11.0.0` to `ec14.0.0` when `TestCreditLedger` is added.

## Boundaries

EC-14 does not add marketplace settlement, real token value, staking, slashing, reward vaults, stake vaults, DAO voting, or reputation-weighted governance.
