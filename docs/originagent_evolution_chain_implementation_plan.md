...
```text
本节覆盖早期 brainstorm 和客户端 Phase 2+ 路线中的旧 token / staking / slash / RewardVault / StakeVault 设想。

EC-15 候选：
  Network Bootstrap / Work Node Registry / Growth Reward Simulator。
  先定义 Genesis Manifest、Endpoint Manifest、Ingress Gateway、Artifact Mirror、Work Node、community evaluation claim 和 Test Credit 增长模拟。
...
EC-18+：
  再评估真实 token、staking、slash、DAO、RewardVault、StakeVault。
```
...
```text
真实 token。
StakeVault / RewardVault / ArenaRegistry / BaselineRegistry / Governor。
OCI push / pull。
Cosign / SBOM 真实验证。
主网部署。
```
...
边界：
  不新增合约、不改变 testnet-only 边界、不提高 evidence 经济权重。
```
...
Validator minimum gate:
  EC-3 先采用基金会 allowlist，不引入 StakeVault。
...
Challenge window:
  先只做状态和事件，不做真实 slash / 罚没。
  被挑战成立的报告标记 invalidated，并降低 EC-3-only validator reputation / weight。
  EC-3 reputation 默认 testnet-only，不直接迁移到未来真实 token / slash 阶段。
```
...
测试覆盖 invalidated report 只触发 EC-3-only 降权，不产生真实 slash。
EC-3 链上/索引数据明确标记 testnet-only；未来主网信誉初始化不得默认继承。
```

## EC-4：Challenge Adjudication + Testnet Reputation
...
边界：
  不新增 token / slash / StakeVault / RewardVault / DAO。
  Testnet reputation 只做审计和负向约束，不产生经济收益，不默认迁移主网。
```
...
代码实现已完成：
  ...
  继续只使用本地 Anvil，不做公网测试网、真实 token、StakeVault、RewardVault、DAO 或 OriginAgent 客户端集成。
...
审计边界：
  ...
  EC-6 不改合约、不引入数据库、不做公网测试网、不做 token/DAO/StakeVault/RewardVault。
...
标准化结果：
  ...
  server-standardization artifact 不写生产密钥、Anvil 私钥、password、token、本地路径或 URL query。
...
边界：
  仍只使用本地 Anvil。
  不做公网测试网、真实 token、StakeVault、RewardVault、DAO、PVE 矩阵或数据库 indexer。
...
边界：
  ...
  不做 token、staking、RewardVault、StakeVault、DAO。
...
叙事：
  ...
  强调不发行真实 token、不在早期启动最终 private appchain、不从零写共识/VM、不做空泛 DAO、不让客户端依赖链运行。
...
兼容性决策：
  ...
  地址级 testnet reputation 继续服务 challenge 流程。
...
核心判断：
  ...
  模块市场和 test credit 应建立在开放 unit kind 协议之上，否则会把市场锁死在早期想象里。
...
建议路线：
  ...
  EC-13: 根据 EC-12 结果收口反女巫、反串谋和审计权重。
  EC-14+: 再推进 Test Credit / 模块市场 MVP。
  更后续再评估真实 token、staking、DAO。
```
...
核心判断：
  不能在没有攻击模拟的情况下直接进入 Test Credit 或模块市场。
...
核心判断：
  ...
  EC-13 是解释层，不是惩罚层；它输出 gate，不写链、不扣分、不 slash、不发 token。
  EC-14 Test Credit / marketplace 应消费 EC-13 的 trust policy report，而不是直接消费裸 abuse signals。
...
建议路线：
  EC-14: Test Credit 沙盒应先读取 EC-13 trust gates。
  模块市场、真实 token、staking、DAO 继续后置。
```

## EC-14：Test Credit Sandbox
...
核心判断：
  ...
  Test Credit 只验证准入、记账、审计和 trust gate 消费路径。
...
部署与兼容：
  ...
  TestCreditLedger 是 deployment 可选合约。
  旧 EC-5 到 EC-13 deployment 不因缺 TestCreditLedger 而失效。
  EC-12 和 EC-13 没有新增合约，因此 contractVersion 从 ec11.0.0 跳到 ec14.0.0。

SDK/CLI：
  新增 sdk/src/test-credit.ts。
  新增 create/validate test credit action。
  新增 create/validate test credit report。
...
边界：
  不做 marketplace。
  不做真实 token。
  不做 staking、slash、RewardVault、StakeVault、DAO。
  不自动扣罚已有余额。
  Test Credit 是审计输入和沙盒记账，不是自动处罚系统。
...
当前进化链已经具备：
  ...
  EC-14 non-transferable Test Credit sandbox。
...
OriginAgent 进化链层：
  逻辑账本层和审计协议层。
  记录公开事实、hash、事件、Passport、module digest、evidence、challenge、reputation checkpoint、unit kind status、trust summary、Test Credit。
...
```text
如果 EC-15 继续作为下一阶段，应先定义 OriginAgent 网络发现、入口 gateway、artifact mirror、社区工作节点身份、活动报告和 Test Credit 增长奖励模拟。
先用 Test Credit 模拟节点扩张激励，不发行真实 token。
```
...
新增 ChallengeAdjudicationRegistry：
  ...
  finalize 通过 VerificationRegistry.resolveChallengeFromAdjudicator 结算 evidence 状态和 testnet reputation。
...
真实 token、staking、slash、DAO、marketplace settlement 仍禁止。
```
...
更新 audit-bundle、chain-state-check、Test Credit bond artifact 和 Trust Policy unresolved dispute。
...
EC-15B audit / state inspection surface 已完成：
  ...
  Test Credit challenge bond 仍是纯链下 artifact，不触发链上 credit movement。
...
EC-15B 当前边界：
  ...
  challenge bond 仍是 Test Credit 链下 artifact，不触发链上 credit movement。
  不引入真实 token、staking、slash、DAO、marketplace settlement、节点挖矿奖励或空投预期积分。
```
...
限制对象应是正式准入主体，而不是任意钱包地址。一个正式 Developer / Validator / Operator 默认最多 3 个 Active Agent：主力 Agent、验证 / 测试 Agent、沙箱 / 实验 Agent。历史 passport 不能删除，应保留审计轨迹；退休 passport 不应继续获得工作权重或 Test Credit 权重。
