...
off-chain Test Credit challenge bond artifacts
```

EC-15B does not add real tokens, staking, slash, DAO governance, marketplace settlement, node mining rewards, or airdrop-like points.
...
Challenge bond remains off-chain Test Credit artifact modeling only.

Audit requires a `challenge_bond_lock` artifact for EC-15B adjudication reports, but there is no on-chain credit movement, forced refund, or enforceable slash.
...
EC-15B is mandatory before any real economic incentive experiment. Even after EC-15B, the system still depends on Foundation allowlist validator admission and off-chain Test Credit modeling.

Do not introduce real token emission, staking, slash, DAO, marketplace settlement, node mining rewards, or airdrop-like points in EC-15B.
