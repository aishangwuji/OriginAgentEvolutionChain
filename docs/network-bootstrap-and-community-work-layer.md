...
Ingress / Gateway endpoint:
  Accepts public artifact envelopes, computes or checks hashes, stores content, returns receipts, and may relay public chain anchors.
...
Community validation can become a new mining form:
...
In the current design stage, these mining terms refer to Test Credit accounting or simulation only. They are not real token emissions, staking rewards, marketplace settlement, or slashable economic finality.
