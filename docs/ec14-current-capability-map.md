...
Baseline commit: `3d2b889 feat: add EC-14 test credit sandbox`

## Purpose

This document records what the evolution chain can actually do after EC-14, what it cannot do yet, and why the next design step should settle network bootstrap, artifact distribution, and community work boundaries before real tokens or marketplace settlement.

EC-14 has produced a working chain-side audit and Test Credit sandbox. It is not a production public network, token economy, marketplace, or DAO.
...
proof bundle
  ...
  -> non-transferable Test Credit grant / consume / deny
```
...
OriginAgent 进化链层:
  ...
  Records public facts, hashes, contract state, chain events, Passport anchors, module digests, evidence, challenges, reputation checkpoints, unit kind status, trust summaries, and Test Credit events.
  ...
  It is not the client runtime, not artifact storage, and not a real token economy.
...
```text
EC-15 不是底层区块链共识节点。
The candidate next phase defines network bootstrap, OriginAgent protocol work nodes, measurable public work, and Test Credit reward simulation.
Node ID 是贡献账户，不是邀请码。
```
...
Test Credit growth reward simulation
```
...
VerificationRegistry:
  ...
  Does not create real slash, staking, or economic finality.
...
TestCreditLedger:
  Stores non-transferable Test Credit accounting by passportId.
...
create and validate Test Credit actions and reports
grant and consume Test Credit through dry-run or broadcast
read module, evidence, challenge, Passport, reputation, unit kind, and Test Credit chain state
```
...
Test Credit grant/consume actions against TestCreditGranted / TestCreditConsumed events
Test Credit deny actions as denied=true and event_found=null
...
Passport Test Credit granted, consumed, balance, and operation count
...
maintain testnet-only address reputation for challenge outcomes
...
grant, consume, or deny non-transferable Test Credit in a sandbox
...
```text
issue real tokens
run staking, slashing, rewards, RewardVault, StakeVault, or DAO voting
...
account for community evaluation mining or challenge mining
...
```text
register node -> receive token over time
```
...
The reward should initially be Test Credit only. Real token, marketplace settlement, staking, slashing, and DAO voting should remain later phases.
...
```text
do not launch real token emissions
...
do not treat Test Credit as money
