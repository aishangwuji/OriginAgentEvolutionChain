...
```text
本文早期章节保留 brainstorm 历史设想，包含真实 token、质押、验证节点裁判、官方 release 签名等方向。
若早期章节与后续章节冲突，以第 30 节“网络发现、躯体分发与社区验证挖矿边界”和
...
当前口径：
  ...
  奖励先用 Test Credit / 模拟账本，不发行真实 token。
...
当前权威路线：
  ...
  EC-18+：再评估真实 token、staking、slash、DAO。
...
- 让优秀模组进入下一代稳定版本。
- 让推动版本跃迁的开发者、社区评估者、试用者先获得 Test Credit、reputation 和 future reward eligibility。
...
跨节点验证
身份信誉
Test Credit / future reward eligibility
擂台赛
基线推进共识
四代版本演进公告
```
...
```text
第一阶段不要启动最终 private appchain / 公链。
第一阶段也不要直接发行真实代币。
...
奖励:
  ...
  不发行真实 token。
  不做复杂 RewardVault / StakeVault。
  无 slash 阶段产生的 reputation 默认 testnet-only，不直接迁移为主网权重。

沙箱:
  MVP 最低实现 Worker 子进程 + Gateway token/time/tool 限制。
...
MVP 不能把“自己生成 fixture、自己合约接受”误认为验证闭环。EC-3 至少需要一份来自非本仓库 CI 的独立机器 validator report，并记录 runner / operator 的公开摘要。没有 StakeVault 前，高权重 validator report 必须通过基金会 allowlist；同一 operator group 或同一 runner fingerprint 的多份报告不能线性叠加。
...
如果一个身份经常给垃圾模块高分，或者恶意差评竞争模块，权重下降，严重时罚没质押。
...
愿意参与进化的用户或 Agent 可以加入 Next 试用计划，通过试用、反馈、评分获得代币。
...
20. 不作为纳什均衡：过强 slash 会诱导验证者拒绝高风险创新，只批准安全但无用模块。
...
如果验证者只因放行后出问题而被 slash，却不会因为错误拒绝优秀创新而承担机会成本，理性策略会变成保守拒绝。
...
no_direct_governance_decision:
  LLM evaluator 不能单独决定 pass / reward / slash / baseline。
...
提交漏洞只能获得有限赏金。
利用漏洞窃取用户资产、企业数据，或做空作者质押，收益可能高出百倍。
理性攻击者会选择利用而不是披露。
```

因此，延迟奖励和 slash 不能替代安全市场设计。
...
no_shorting_during_disclosure:
  漏洞提交者和验证委员会在披露窗口内不得针对相关质押 / 市场头寸获利。
...
```text
本周有 3 个候选模组通过本地沙箱回测。
其中 memory-compressor@2.1 在你的历史任务影子副本上节省约 18% token，
...
如果直接按回滚率 slash，会误伤优秀开发者，导致没人愿意开发外部集成模组。
...
如果模块被调用越多、组合采用越多就获得更多代币，必然出现合谋。
...
```text
Gateway 按 module_id / artifact_digest / activation_id 维护 token 计数。
预算来自 manifest.context_budget.token_budget。
单次请求和累计请求都要 pre-flight。
Provider response 回来后 post-flight 记录真实 usage。
超预算立即返回 TokenLimitExceeded。
...
使用 restricted token / AppContainer 能力时优先启用。
...
无 slash 阶段产生的 reputation 默认不迁移到未来真实经济阶段。
```
...
主叙事：
  ...
  不是普通插件市场，不是去中心化 RAG 数据库，也不是先发 token 的链上项目。
...
裁剪原则：
  ...
  强调野心大但执行克制：不首发真实 token、不在早期启动最终 private appchain、不从零写共识/VM、不做空泛 DAO、不让客户端依赖链运行。
...
边界保持：
  ...
  不建数据库或 explorer。
  不接公网测试网。
  不引入真实 token、DAO、StakeVault、RewardVault。
...
边界保持：
  不做公网测试网。
  不引入真实 token、StakeVault、RewardVault、DAO。
...
反女巫方向：
  不追求链上硬性证明“一人一 Agent”。
  新 Agent 默认低权重、无自动收益。
  高权重来自贡献历史、验证准确率、挑战记录、社区背书、质押和多样性约束。
  同 owner / operator group / runner fingerprint 下的多个 Agent 应有收益和权重递减。

代币经济边界：
  不设计注册即每日领币的水龙头。
  初期只考虑测试积分 / test credit。
  模块包早期由作者定价，协议收小额手续费，验证者可分享验证收益。
  拍卖、DAO 定价、真实 token、质押和奖励放到公网测试网滥用数据之后。
...
建议路线：
  ...
  EC-14+: 再评估 Test Credit、模块市场、真实 token、staking、DAO。
...
本阶段不新增合约，不做数据库/对象存储，不做公网测试网，不发行 token。
...
边界：
  不做 token、staking、slash、RewardVault、StakeVault、DAO。
  不做自然人唯一性证明。
  不自动合并同 owner 多个 Passport 的声誉。
  声誉碎片化和 Sybil 风险留给后续 test credit / 反女巫阶段通过审核、衰减和经济约束处理。
```
...
```text
EC-11 的重点从“先做 Test Credit + 模块市场 MVP”调整为“先打开 evolution unit 的类型边界”。
...
```text
Test Credit 和模块市场应在 EC-11 之后推进。
...
```text
EC-12 的重点从“准备 Test Credit / 模块市场”前移为“先模拟攻击和滥用”。
...
EC-12 不自动 slash、不扣分、不奖励。
Abuse report 只是审计信号，为后续 Test Credit / marketplace 提供测试数据集。
```

对后续路线的影响：

```text
Test Credit 和模块市场应在 EC-12 之后推进。
...
核心意义：
  ...
  这一步仍然不自动惩罚，只给 EC-14 Test Credit / marketplace 提供可审计输入。
```
...
```text
EC-14 Test Credit 不应直接基于 Passport 数量、单一 reputation 分或单一 validator 报告发放。
EC-14 应先消费 trust_policy_report，把 high/critical 风险主体排除或转人工复核。
模块市场、真实 token、staking、slash、DAO 继续后置。
```
...
## 28. EC-14 Test Credit Sandbox / 激励前积分沙盒

2026-05-23 更新：

```text
EC-14 在 EC-13 trust gate 之后新增非转让 Test Credit 沙盒账本。
...
```text
TestCreditLedger 合约只做 owner-only 和账本完整性校验。
...
```text
grant / consume 必须有链上 TestCreditGranted / TestCreditConsumed 事件。
deny action 不广播链上交易。
audit-bundle 对 deny action 标记 denied=true、event_found=null，不把缺事件视为错误。
chain-state-check --passport-id 可读 Test Credit granted / consumed / balance。
```
...
真实 token、staking、slash、DAO 继续后置，直到沙盒市场有足够滥用数据。
```

实施记录：

```text
OriginAgentEvolutionChain 已新增 TestCreditLedger。
CLI 新增 create/validate test credit action/report、grant-test-credit、consume-test-credit。
...
当前进化链已经能完成：
  ...
  abuse report -> trust policy -> Test Credit sandbox。
...
先用 Test Credit 模拟节点增长激励。
不发行真实 token。
不做 marketplace settlement。
不做 staking / slash / DAO。
...
这不是官方任务队列，而是一种新的 Evaluation Mining / Validation Mining / Audit Mining。
当前挖矿/奖励语义只指 Test Credit 或模拟账本，不指真实 token emission。
...
公开网络前必须推进 EC-15B：
  ...
  challenge bond 先用 Test Credit artifact 模拟。
...
```text
EC-15B 完成并验证前，不发行真实 token。
EC-15B 完成并验证前，不做空投预期积分。
EC-15B 完成并验证前，不做节点挖矿奖励。
EC-15B 完成并验证前，不做 staking / slash / DAO / marketplace settlement。
```
...
仍未完成的 EC-15B 后续项：
  ...
  Test Credit challenge bond artifact 和 Trust Policy unresolved dispute。
...
EC-15B audit / chain-state hardening 已完成：
  ...
  challenge bond 仍是 Test Credit 链下 artifact；缺少 challenge_bond_lock artifact 时 EC-15B adjudication audit 失败，但不触发链上扣款。
...
不做空投预期积分。
不做节点挖矿奖励。
不做 staking / slash / DAO / marketplace settlement。
不把 operatorGroupHash / runnerFingerprintHash 当作合约级 Sybil 防御。
不把 Test Credit challenge bond 当作真实可强制退款或罚没。
```
